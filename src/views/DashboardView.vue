        ...
        <div class="grid grid-cols-8 mt-2 gap-4 pb-1.5">
          ...
          <div class="flex items-center">
            <button
              ...
              class="rounded-md font-semibold px-3 py-2 bg-gray-100 hover:bg-gray-200"
            >
              Add Privilages
            </button>
          </div>
...
export default defineComponent({
  setup() {
    ...
    const createClient = async () => {
      ...
      if (post.status !== 200) {
        hasNotification.value = true;
        notificationMessage.value = "An error occred when processing request";
        return;
      }
      const response = await post.json();
      hasNotification.value = true;
      notificationMessage.value = "Client successfully created";
    };
